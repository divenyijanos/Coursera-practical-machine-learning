...
The model's performance on the validation set is quite poor. However,
it is still a large improvement compared to the benchmark (we correctly
specified the classe in 40% of the cases whereas there are 5 classes with close
to equal prevalence).
...
a further improvement in accuracy reaching 99% on the validation set. 
...
nonmiss.forest.all <- train(classe ~ .,
                            ...
                            method="rf")
# performance on validation set
mean(predict(nonmiss.forest.all, validation) == validation$classe)
```
...
We can see that the models which use all of the variables perform better than
the preprocessed one even on the validation set. As ensembling different models
together always promises some accuracy gain (if there is at least some
indepence between the models), I stack together the last three models by using
the random forest method on the predictions. Not surprisingly, this model
reaches the highest accuracy on the validation set amounting to  percent.


```{r stack together}
df.predicted <- data.frame(
    pred.forest=predict(nonmiss.forest, all.nonmiss$training),
...
confusionMatrix(predict(stacked, df.predicted.validation),
                validation$classe)
```
...
precise predictive model. As the last step I use my best model (the stacked
one) to predict the test cases.
...
testing[gyros_variables] <- log(testing[gyros_variables] + 500)
df.predicted.test <- data.frame(
    pred.forest=predict(nonmiss.forest.pca, testing),
    pred.forest.pca=predict(nonmiss.forest.all, testing),
    pred.boost=predict(nonmiss.boost, testing),
)
as.data.frame(predict(stacked, df.predicted.testing))
```
